...
### Hitrates
#### Display of hit-rate distributions
...
#### Display of smoothed Hit rates across time and experimental conditions
What are the time courses of  detected primed and nonprimed events?
...
ggplot(data_bin, aes(x = eventonsettimes, y = hit)) +
  stat_summary(fun = mean, geom = "point", alpha = 0.4) +  # mean RTs
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs"), se = TRUE) +
  labs(x = "Event Onset Time", y = "Probability of hits", 
       title = "GAM Smoothed Probability of Hits") +
  theme_minimal()
```
<br> 
...
# The y-axis shows the linear predictor — that is, the log-odds of the outcome ("hit") as predicted by the model.
# 
# This is before applying the inverse link function (which would convert it to a probability).
# 
# That means:
# Y-axis = logit(P(hit))
# 
# Values:
# 
# 0 → 50% chance of a hit
# 
# Positive values → >50% chance
# 
# Negative values → <50% chance



eventonset_grid <- seq(min(DATA_Trials$eventonsettimes),
                       max(DATA_Trials$eventonsettimes),
                       length.out = 200)

new_target_primed <- data.frame(eventonsettimes = eventonset_grid,
                    eventtype2 = "target_primed",  # match your factor levels exactly
                    participant = 1)

new_target_nonprimed <- data.frame(eventonsettimes = eventonset_grid,
                    eventtype2 = "target_nonprimed",
                    participant = 1)

pred_target_primed <- predict(model_gam_interaction, newdata = new_target_primed, type = "response")
pred_target_nonprimed <- predict(model_gam_interaction, newdata = new_target_nonprimed, type = "response")

prob_diff_df <- data.frame(
  eventonsettimes = eventonset_grid,
  prob_target_primed = pred_target_primed,
  prob_target_nonprimed = pred_target_nonprimed,
  prob_diff = pred_target_primed - pred_target_nonprimed
)

ggplot(prob_diff_df, aes(x = eventonsettimes, y = prob_diff)) +
  geom_line() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(y = "Difference in predicted probabilities (pred_target_primed - pred_target_nonprimed)",
       x = "Event onset time")
...
<br> 
```{r stat_hitxpresentationtime_distr_contrasts, warning = FALSE, fig.height=4, fig.width=7}
# Full model with interaction
model_gam_interaction <- DATA_Trials %>%
  mutate(hit = ifelse(response == "hit", 1, 0),
         eventtype2 = as.factor(eventtype2))%>%
...
early_window <- c(0, 0.5) + min(DATA_Trials$eventonsettimes)
late_window <- c(-0.5, 0) + max(DATA_Trials$eventonsettimes)
...
DATA_Trials %>%
  ...
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs"), se = TRUE, aes(group = eventtype2)) +
  ...
  geom_point(data = emm_predictions_plot, aes(y = prob, group = eventtype2), size = 3, shape = 23, stroke = 1.5) +
  ...
  annotate("segment", x = early_window[1], xend = early_window[2], y = 0.3, yend = 0.3, color = "grey60", size = 1.5) +
  annotate("segment", x = late_window[1], xend = late_window[2], y = 0.3, yend = 0.3, color = "grey60", size = 1.5) +
  theme_minimal()
...
#### Display of smoothed reaction times across time
...
# contrast of factor eventtype2
emm_LMRT_full <- emmeans(LMRT_full, ~ eventtype2 | eventonsettimes,
                         at = list(eventonsettimes = seq(min(DATA_Trials$eventonsettimes), max(DATA_Trials$eventonsettimes), length.out = 10)))
contrast_results <- contrast(emm_LMRT_full, method = "pairwise", adjust = "tukey")
# summary(contrast_results)
results_df <- as.data.frame(contrast_results)
significant_times <- subset(results_df, p.value < 0.05)

ggplot(results_df, aes(x = eventonsettimes, y = estimate)) +
  geom_line() +
  geom_ribbon(aes(ymin = estimate - SE, ymax = estimate + SE), alpha = 0.2) +
  geom_point(data = significant_times, color = "red", size = 2) +
  labs(title = "Difference between eventtype2 levels over time",
       y = "Estimated difference (RT)", x = "Event onset time")
...
ggplot(diff_df, aes(x = eventonsettimes, y = .diff)) +
  ...
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci), alpha = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Event onset time", y = "Difference in RT (nonprimed - primed)")
```
...
<br> 
```{r stat_RTxpresentationtime_distr_contrasts, warning = FALSE, fig.height=4, fig.width=7}
# Full model with interaction
# Model with smoothers by condition
model_gam_interaction <- DATA_Trials %>%
  filter(response=='hit')%>%
...
DATA_Trials %>%
  ...
  geom_point(data = emm_predictions_plot, aes(y = emmean, group = eventtype2), size = 3, shape = 23, stroke = 1.5) +
  geom_errorbar(data = emm_predictions_plot, aes(y = emmean,ymin = lower.CL, ymax = upper.CL), width = 0.1, size = 1) +
  # Add vertical lines for the early time window
  annotate("segment", x = early_window[1], xend = early_window[2], y = 500, yend = 500, color = "grey60", size = 1.5) +
  annotate("segment", x = late_window[1], xend = late_window[2], y = 500, yend = 500, color = "grey60", size = 1.5) +
  theme_minimal()
...
# Model with smoothers by condition
model_gam_factor <- DataIn1%>%
  filter(eventtype=="distractor")%>%
  mutate(FA = ifelse(response == "FA_proper", 1, 0))%>%
  gam(FA ~ s(eventonsettimes) + s(participant, bs = "cs"), 
                 data = ., family = binomial)

model_gam_null <-DataIn1%>%
  filter(eventtype=="distractor")%>%
  mutate(FA = ifelse(response == "FA_proper", 1, 0))%>%
  gam(FA ~ s(participant, bs = "cs"), 
                 data = ., family = binomial)
...
summary(contrast_tdiffs) %>%
  ...
  kbl(escape = F, digits=4,
      caption = paste0("Contrast of marginal means of RTs between early and late time window"))%>%
        # caption = paste0("Likelihood ratio test via anova | comparing full and reduced model | GAM:<br>", 
        #             paste(deparse(formula(model_gam_interaction)), collapse = " ")))%>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8,latex_options = "hold_position"))
...
summary(contrast_condiffs) %>%
  ...
  kbl(escape = F, digits=4,
      caption = paste0("Contrast of marginal means of RTs between conditions"))%>%
        # caption = paste0("Likelihood ratio test via anova | comparing full and reduced model | GAM:<br>", 
        #             paste(deparse(formula(model_gam_interaction)), collapse = " ")))%>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8,latex_options = "hold_position"))
