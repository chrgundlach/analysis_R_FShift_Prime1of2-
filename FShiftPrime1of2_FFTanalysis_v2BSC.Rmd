...
for BSC thesis with smaller and more time windows
...
DATAPath = "data_in/FFT_Amp_data_CenterLarge_ShorterTimeWins09-25-2025_10-40.csv"
DATAPathGabor = "data_in/Gabor_Amp_data_CenterLarge_ShorterTimeWins09-25-2025_13-07.csv"
t_baseline = c(-1000, 0)
...
# read in data Gabor
DataInGabor <- read.csv(DATAPathGabor, sep = ";") %>%
  mutate(subjects = as.factor(subjects))%>%
  filter(!subjects %in% sub_sample)%>%
  mutate(condition = factor(condition, levels=c('primed','nonprimed','not attended'), ordered = T),
         time = factor(time, levels=c('[250 1000]ms','[1000 1750]ms'), ordered = T))
head(DataInGabor)
str(DataInGabor)
```
...
ggplot(dat2plot, aes(x = interaction(time,RDK_isprimed), y = amplitude_evoked, fill = RDK_isprimed)) +
  ...
  scale_x_discrete("time windows in s", breaks=waiver(), labels = rep(c("[-0.75 0]","[0.25 1]","[1 1.75]"),1,9)) +
...
plotttitle <- ggdraw() + 
  draw_label("SSVEP mod. | evoked \n'[0.25 1.0] s", fontface='bold', size = 10)
...
# cowplot::plot_grid(plot1, plot2, plot3, ncol = 3,labels = "AUTO")
plotttitle <- ggdraw() + 
  draw_label("SSVEP mod. | evoked \n'[1 1.75] s", fontface='bold', size = 10)
plotplots <- ggarrange(plot1, plot2,ncol = 2,common.legend = TRUE,align = "h", widths=c(3,1))
...
### 1c  Illustrate Data as is | center RDKs from Gabor data

plot differently

<br>  

```{r plot_data_st_center_2_Gabor, results = "hide",  fig.height=5, fig.width=4, warning = FALSE}
#evoked raw#### for center

# set overall limits?
dat2plot <- DataInGabor %>%
  group_by(subjects,  time, condition)%>%
  summarise(amplitude_evoked=mean(amplitude_evoked), modulation_evoked=mean(modulation_evoked))%>%
  filter(time=='[250 1000]ms')
# set limits
yax_limits <- c( min(dat2plot$modulation_evoked), max(dat2plot$modulation_evoked))
yax_limits <- round(yax_limits + c(-1, 1)*diff(yax_limits)*0.1)
yax_limits <- yax_limits + c(-1, 1)*diff(yax_limits)*0.1
...
theme_set(theme_bw())
plot1 <- ggplot(dat2plot, aes(x = condition, y = modulation_evoked, fill = condition)) +
  geom_hline(yintercept=0, show.legend = FALSE) +
  geom_line(aes(group = interaction(subjects,time)),colour = "grey60",alpha = 1,size =0.5) +
  geom_beeswarm(aes(color = condition, x = condition,
                    group = condition), cex=3, size = 5,alpha=1,fill="grey40",shape=21)+
  # geom_point(aes(color = RDK_isattended, x = interaction(time,RDK_isattended),
  #                   group = RDK_isattended), size = 4,alpha=0.4)+
...
plot2 <- ggplot(dat2plot, aes(y = modulation_evoked, fill = condition, x = 1)) +
...
```{r Stat central stimuli Gabor, message=FALSE, warning=FALSE, include=TRUE}
dat2plot <- DataInGabor %>%
  group_by(subjects, time, condition)%>%
  summarise(amplitude_evoked=mean(amplitude_evoked), modulation_evoked=mean(modulation_evoked))%>%
  rename(timewindow = time)
...
dat2plot %>%
  ...
  unnest(cols = c(stats, t_test_bf, meanval, std))%>%
  mutate(BF10 = bf, BF01 = 1/bf)%>%
  dplyr::select(timewindow, condition, meanval, std,  statistic, parameter, p.value, BF10, BF01)%>%
  ungroup()%>%
  group_by(timewindow)%>%
  # mutate(p.value = p.adjust(p.value, method="holm")) %>% # none, holm
  # mutate(p.value = p.adjust(p.value, method="bonferroni")) %>% # none, holm
...
dat2plot %>%
  dplyr::select(-amplitude_evoked)%>%
  pivot_wider(names_from = "condition", values_from = "modulation_evoked")%>%
  mutate(primedMINUSnonprimed = `primed` -`nonprimed`, 
         primedMINUSunattended = `primed` -`not attended`, 
         nonprimedMINUSunattended = `nonprimed` -`not attended`)%>%
  dplyr::select(-`primed`, -`nonprimed`, -`not attended`)%>%
