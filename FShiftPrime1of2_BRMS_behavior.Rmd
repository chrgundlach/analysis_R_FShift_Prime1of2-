...
ggplot(dat_plot, aes(x=RT)) +
  ...
  labs(
    ...
    subtitle = "all trials for N = 27 subjects",
    x = "RT in ms",
    y = "density"
  )+ 
...
plot2 <-
  ...
  theme(axis.text.y=element_blank(),axis.ticks.y=element_blank(), axis.title.y = element_blank())+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  scale_x_continuous(name="RT in ms", breaks=c(200,400,600,800,1000,1200), labels=waiver(), limits=c(200,1300))
...
# plot difference scores across draws
bestmodel.ef.tidy_exppred2b <- bestmodel.ef.tidy_exppred2 %>%
  ungroup()%>%
  dplyr::select(eventtype2,eventonsettimes,subject,.draw,.epred,-".row")%>%
  pivot_wider(names_from = eventtype2, values_from = ".epred")%>%
  mutate(diff_primed_nonprimed = target_primed-target_nonprimed) %>%
  dplyr::select(-target_primed,-target_nonprimed)%>%
  mutate(contrast = "primed - nonprimed")
  

theme_set(theme_bw())
ggplot(bestmodel.ef.tidy_exppred2b, aes(x = eventonsettimes, y = diff_primed_nonprimed)) +
  stat_lineribbon(.width = c(0.95, 1),aes(fill = contrast, fill_ramp = after_stat(level)), alpha = 0.5) + 
  # scale_fill_brewer(palette = "Reds") +
  scale_fill_manual(values=c("mediumpurple3")) +
  # scale_color_manual(values=c("red3", "steelblue"))+
  # facet_grid(.~eventtype2)+
  labs(x = "event onset times", y = "predicted RT difference",
       # fill = "Credible interval",
       title = "Marginal Means of differences", 
       subtitle = "for all draws of the model",) +
  # ylim(c(400,1300))+
  theme(legend.position = "bottom")
...
g<-ggplot(bestmodel.ef.slope_interaction, aes( x = .value)) +
  ...
  # scale_fill_brewer(palette = "Reds")+
  labs(
    title = "Contrast of estimated slopes", 
    subtitle = "based on medians for all draws of the model",
    fill_ramp = "interval",
    x = "predicted difference in ms",
    y = "density"
